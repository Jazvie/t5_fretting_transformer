#!/usr/bin/env python3
"""
Retrain SynthTab Model WITHOUT Capo/Tuning Conditioning

This trains the model in standard tuning (capo=0) only, without any conditioning.
Uses pre-made train/val splits for reproducibility.

Key configuration:
- enable_conditioning=False (NO capo/tuning augmentation)
- Standard tuning only (64,59,55,50,45,40)
- Capo 0 only
- Uses pre-made splits from synthtab_acoustic_train.jsonl and synthtab_acoustic_val.jsonl
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        default="checkpoints_no_conditioning",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to checkpoint to resume training from (e.g., checkpoints_no_conditioning/checkpoint-12345)",
    )
    parser.add_argument(
        "--batch-size",
        type=int,
...
def main(args: argparse.Namespace) -> None:
    """Retrain SynthTab model WITHOUT conditioning (standard tuning, capo 0 only)."""

    print("SynthTab Training WITHOUT Capo/Tuning Conditioning")
    print("=" * 70)
    print("NO CONDITIONING: Standard tuning (capo=0) only")
    print("No data augmentation - 1x examples (vs 8x with conditioning)")
    print("Using pre-made train/val splits for reproducibility")
    print("=" * 70)
    ...
    print(f"  - Model: {args.model_name_or_path if args.use_pretrained else 'tiny scratch'}")
    print("=" * 70)
    ...
    # Data configuration - NO CONDITIONING
    print("\nLoading pre-made train/val splits (NO CONDITIONING MODE)...")
    data_config = DataConfig(
        ...
        # Conditioning parameters below are ignored when enable_conditioning=False
    ...
    training_config = TrainingConfig(
        ...
        warmup_steps=args.warmup_epochs * 2375,  # Calculate from warmup epochs
        num_train_epochs=args.epochs,  # Use command line epochs
        logging_steps=100,
        save_total_limit=5,
